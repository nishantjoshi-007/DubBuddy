...
def validate_upload(path: Path | str, settings: Settings) -> Probe:
    ...
    if size_mb > settings.max_upload_mb:
        raise InputError(
            f"The upload is {size_mb:.0f} MB; the limit is {settings.max_upload_mb} MB."
        )
    try:
...
def extract_audio(source_mp4: Path | str, dest_dir: Path | str) -> tuple[Path, Path]:
    ...
    ffmpeg.run(
        [
            "-i", str(source),
            "-vn", "-ac", "1", "-ar", str(SOURCE_SAMPLE_RATE), "-c:a", "pcm_s16le",
            str(source_wav),
        ]
    )
    ...
    ffmpeg.run(
        [
            "-ss", f"{start:.3f}", "-t", f"{window:.3f}", "-i", str(source),
            "-vn", "-ac", "1", "-ar", str(REFERENCE_SAMPLE_RATE), "-c:a", "pcm_s16le",
            str(reference_wav),
        ]
    )
    if not reference_wav.exists() or reference_wav.stat().st_size == 0:
        ...
        ffmpeg.run(
            [
                "-i", str(source_wav),
                "-t", f"{window:.3f}",
                "-ac", "1", "-ar", str(REFERENCE_SAMPLE_RATE), "-c:a", "pcm_s16le",
                str(reference_wav),
            ]
        )
